...
| **情况二** | 附录内是连续的说明文字（无明确子结构） | 在最近的**句号（。）处切分**，切分后每个分段控制在 **600~1000 字符**，并添加 **overlap 100 字符**（`chunk_overlap`，与正文 3.4 情况二一致）。 |
...
```json
{
  "content": "附 录 A（资料性）入口外观效果示例\n入口外观效果示例见图A.1。![](images/60de63a2...jpg)\n\n附 录 B（资料性）候诊区效果示例\n候诊区效果示例见图B.1。![](images/9c1483e7...jpg)",
  "metadata": {
...
### 规则 5.7：表格处理（★ 2026-08-13 新增）

- 表格（MinerU `table` 类型块）**保留 HTML `<table>` 原文**（含 `<tr>` / `<td>` / `<th>` 标签），与相邻正文合并为一个分段，不破坏表格结构。
- **表格独立成段**：当表格行数超过 `chunk_table_row_threshold`（默认 20 行）或 HTML 长度超过 `chunk_table_max_chars`（默认 5000 字符）时，将表格**独立为一个分段**（`chunk_type="table"`），前后正文不再与它合并。
- 表格分段同样参与图注 / 前后文合并规则，并计入单分段图片上限（`chunk_max_images_per_segment`）。
